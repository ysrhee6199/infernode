...
Yolov7_2::Yolov7_2(ICudaEngine* engine_)
: Node("yolov7_2")
{
    // QoS
    rclcpp::QoS system_qos = rclcpp::QoS(rclcpp::SystemDefaultsQoS());
    ...
    // Subscriber
    using std::placeholders::_1;
    this->raw_image_subscriber_ = this->create_subscription<sensor_msgs::msg::Image>("/image_raw2", system_qos, std::bind(&Yolov7_2::image_callback, this, _1));
    this->engine_ = engine_;
    std::cerr << "here?\n";
    // Initialize model
    this->initialize_model();
...
void Yolov7_2::initialize_model()
{
...
void Yolov7_2::image_callback(const sensor_msgs::msg::Image::SharedPtr image)
{
    // Convert Ros2 image to OpenCV image
    cv_bridge::CvImageConstPtr cv_image = cv_bridge::toCvShare(image, "bgr8");
    if (cv_image->image.empty()) return;
    ...
    for (size_t j = 0; j < image_batch.size(); j++)
    {
        cv::imshow("Result image2", image_batch[j]);
    }
